...
const Hero = (hero) => (
  <>
    <div className="__dumi-default-layout-hero" style={{ ...hero?.style }}>
...
const Layout: React.FC<IRouteComponentProps> = (props) => {
  ...
  const { mode, repository } = config;
  const { url: repoUrl, branch, platform } = repository;

  // console.log('meta',props,config,repository)
  ...
  return (
    ...
    >
      ...
      <div className="__dumi-default-layout-content">
        {/* <div className="__dumi-default_meta_list">
          {meta?.author && <span className="meta__item meta__item___text">{meta?.author}</span>}
          {meta?.date && <em className="meta__item meta__item___text">{meta?.date}</em>}
        </div> */}
        {children}
        {!showHero && !showFeatures && meta.filePath && !meta.gapless && (
          <div className="__dumi-default-layout-footer-meta">
            ...
            {/* {
              meta?.date && <span data-update-text={isCN ? '创建时间' : 'Create Time: '}>{meta.date}</span>
            } */}
